...
export const applyMutation = (
  ...
  return ports.reduce<Port[]>((acc, port) => {
    if (port.id === mutation.targetId) {
      const copy: Port = { ...port, children: [...(port.children || [])] };
      ...
      const changed =
        newChildren.length !== port.children.length ||
        newChildren.some((c, i) => c !== port.children[i]);
      if (changed) acc.push({ ...port, children: newChildren });
      else acc.push(port);
    } else {
      acc.push(port);
    }
